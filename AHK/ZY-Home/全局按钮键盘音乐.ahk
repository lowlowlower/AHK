...
;
; ======================================================================
; 使 用 Ctrl + Alt + T 快捷键：切换当前窗口的置顶状态
^!t::
{
    ; 获取当前活动窗口的句柄 (HWND)
    hwnd := WinExist("A")
    ...
    ; 检查窗口是否已经处于置顶状态
    ; WinGetExStyle 获取窗口的扩展样式，0x8 是 WS_EX_TOPMOST 的值
    isTopmost := WinGetExStyle(hwnd) & 0x8

    ; 使用 DllCall 调用 Windows API 的 SetWindowPos 函数来改变窗口 Z 顺序
    ; 这是最稳定可靠的置顶方法
    if isTopmost
    {
        ; 如果已置顶，则取消置顶
        ; -2 (HWND_NOTOPMOST) 表示将窗口置于所有非置顶窗口之上
        DllCall("SetWindowPos", "Ptr", hwnd, "Ptr", -2, "Int", 0, "Int", 0, "Int", 0, "Int", 0, "UInt", 3) ; 3 = SWP_NOSIZE | SWP_NOMOVE
        ToolTip("窗口置顶已取消")
    }
    else
    {
        ; 如果未置顶，则设置置顶
        ; -1 (HWND_TOPMOST) 表示将窗口置于所有窗口之上
...
^!v::
{
    ...
    scrcpyPath :="C:\Users\admin\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Visual Studio Code\Visual Studio Code.lnk"
